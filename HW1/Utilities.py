...
# def term_ent_zero(features, label, layer, add):
#     if layer > 6:
#     # if len(label) <= add or compute_entropy(label) == 0:
#         return (True, find_majority(label))
#     else:
#         return (False, None)
#     # if compute_entropy(label) == 0:
#     #     return (True, label[0])
#     # else:
#     #     return (False, None)
...
def split_on_mse(features, label, threshs, layer, term_con, term_thresh):
    ...
    if term_con == c.ERROR_DEC and cur_err - min_err < add:
        return (None, np.mean(label)), None, None

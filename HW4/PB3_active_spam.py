...
def main():
    # training parameter
    round_limit = 50
    result_path = 'results/spamActive_final_1.acc'
    ...
    c = 5
    delta_c = 2
    tr_data = data_set.random_pick(c)
    while True:
        ...
        while round < round_limit: # and not converged:
            ...
            print('Data {}% Round: {} Feature: {} Threshold: {} Round_err: {:.12f} Train_err: {:.12f} Test_err {} AUC {}'.format(c, round, c_f_ind, c_thresh, c_model_err, c_tr_err, c_te_err, 0))
            # converged =  abs(c_te_auc - te_auc) / te_auc <= tol
            # te_auc = c_te_auc

        training_errs.append(round_tr_err[-1])
        testing_errs.append(round_te_err[-1])
        # TODO select new data
        if c <= 50 - delta_c:
            c += delta_c
            tr_data = data_set.active_pick(delta_c, boost)
        elif c < 50:
            tr_data = data_set.active_pick(50 - c, boost)
            c = 50
        else:
            break
        # break      # for testing

...
layer_thresh = 2
T = 50
# result_path = 'results/spamDTBagging_compare_final.acc'
result_path = 'results/spamDT_compare_final.acc'
...
for i in range(1):
    ...
    # while round < T:
        ...
        b_tr_data = tr_data
        bagging.add_tree(b_tr_data[0], b_tr_data[1], threshes, layer_thresh, tree.DecisionTreeClassifier)
        r_model_acc = bagging.test_with_model(-1, b_tr_data[0], b_tr_data[1], util.acc)
        r_tr_acc = bagging.test(tr_data[0], tr_data[1], util.acc)
        r_te_acc = bagging.test(te_data[0], te_data[1], util.acc)
        print('Round {} with model acc: {} training acc: {}, testing acc: {}.'.format(round, r_model_acc, r_tr_acc, r_te_acc))
